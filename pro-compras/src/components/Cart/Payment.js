...
import { userEmail} from "../Account/UserInfo";
import { getMeApi, updateUserApi } from "../../api/user";


export default function Payment(props) {
  const { totalPayment, selectedAddress, products, user } = props;
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formData) => {
      ...
      if (result?.error) {
        ...
        console.log(responsee.email);

        console.log(response + "sudii");
        console.log(product.title + "sudi4");
        console.log(quantity + "sudi3");
